...
namespace Gecko
{
    void PatrolOrderStrategy::execute(const OIS::MouseEvent& arg)
    {
        ...
        for (const Id& object_id : *(player->get_selected()))
        {
            ...
            if (object_result)
            {
                if (OrderPtr order = OrderManager::getSingleton().order_patrol(Id::Empty, object_id, object_result->first))
                {
            ...
            {
                if (OrderPtr order = OrderManager::getSingleton().order_patrol(Id::Empty, object_id, terrain_result->second))
                {
                    object->get_orders()->add_last(order->get_id());
                }
                else
