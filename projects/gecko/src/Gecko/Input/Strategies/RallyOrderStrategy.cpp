...
namespace Gecko
{
    void RallyOrderStrategy::execute(const OIS::MouseEvent& arg)
    {
        ...
        for (const Id& object_id : *(player->get_selected()))
        {
            ...
            if (object_result)
            {
                if (OrderPtr order = OrderManager::getSingleton().order_rally(Id::Empty, object_id, object_result->first))
                {
